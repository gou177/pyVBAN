...
if pyaudio:
    ...
    class VBAN_Recv(object):
        ...
        def __init__(self, senderIp, streamName, port, outDeviceIndex, verbose=False):
            ...
            self.stream = self.p.open(
                ...
                rate=self.sampRate,
                output=True,
                output_device_index=self.outDeviceIndex,
            )
            self.rawPcm = None
            self.running = True
    ...
    class VBAN_Send(object):
        ...
        def __init__(
            self, toIp, toPort, streamName, sampRate, inDeviceIndex, verbose=False
        ):
            ...
            self.stream = self.p.open(
                ...
                rate=self.samprate,
                input=True,
                input_device_index=self.inDeviceIndex,
        ...
        def _constructFrame(self, pcmData):
            ...
            header += bytes(
                self.streamName + "\x00" * (16 - len(self.streamName)), "utf-8"
            )
